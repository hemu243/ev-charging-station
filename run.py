...
class Runner(object):
	...
	def run(self):
		...
		print path.Path.get_direction(data[0]["Full Address"], data[1]["Full Address"], self.api_key)


def parse_args():
	parser = argparse.ArgumentParser()
	parser.add_argument('-t', '--threshold', help="threshold distance between points", required=True, type=float)
	parser.add_argument('-a', '--api_key', help="Google api key for distance matrix,"
												" refer - https://developers.google.com/maps/documentation/distance-matrix/get-api-key for more infromation", required=True, type=str)
